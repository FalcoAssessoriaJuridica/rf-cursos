...
import { Plus, ChevronLeft, Pencil, Trash2, Video, List, X, GripVertical, ChevronDown, ChevronRight, Info, AlertTriangle } from 'lucide-react';
    ...
    const handleLessonSubmit = async (e) => {
        ...
        try {
            ...
            const payload = {
                ...
                // We keep video_provider as a helper column if it exists, or just rely on the values
                // Based on DB check, we don't have a 'video_provider' column, but we might want to store it 
                // However, let's stick to the columns we verified: url_video_drive, id_video_profissional
            };
            ...
            {isPreviewModalOpen && previewLesson && (
                <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-[60]">
                    <div className="w-full max-w-4xl space-y-4">
                        ...
                        {previewLesson.url_video_drive && (
                            <div className="bg-white/5 border border-white/10 rounded-lg p-3 flex items-center justify-between">
                                <span className="text-xs text-text-muted">Link original do Drive:</span>
                                <a
                                    href={previewLesson.url_video_drive}
                                    target="_blank"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-xs text-text-muted uppercase font-bold tracking-wider">
                                            {lessonForm.video_provider === 'google_drive' ? 'Link do Drive' : 'ID do Vídeo'}
                                        </label>
                                        <Input required value={lessonForm.video_id} onChange={e => setLessonForm({ ...lessonForm, video_id: e.target.value })} />
                                    </div>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs text-text-muted uppercase font-bold tracking-wider">Ordem</label>
